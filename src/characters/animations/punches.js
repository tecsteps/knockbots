...
export const PUNCH_CLIPS = {
  ...
  'p.straight': {
    ...
    tracks: {
      ...
      // THE LEAD ARM IS A GUARD RETRACTION, NOT A PASSENGER. See the note at the
      // foot of this file. Measured chest-local — which removes the torso carry
      // entirely — the old lead hand sat within 50mm of one point for ticks 0-12,
  ...
  'p.uppercut': {
    ...
    tracks: {
      ...
      // THE LEAD ARM SINKS WITH THE BODY AND THEN COVERS. Same reasoning as
      // p.straight -- see the note there and at the foot of this file -- but a
      // DIFFERENT route through the reach, because the two moves must not park
      // the off-hand in one place. The pelvis drops 211mm on this clip and the
      // lead hand goes down WITH it and across the centreline (chest-local
      // y -110 at the chamber, where p.straight's lowest is +75), then rides the
      // launch all the way to a high tight cover at y +287. That vertical sweep
      // is the widest of any punch here, which is right: an uppercut exposes the
      // chin further than anything else in the file and the lead hand is the only
...
// ---------------------------------------------------------------------------
// THE OFF-ARM WAS A PASSENGER, AND THE CRITIC WAS RIGHT FOR THE WRONG REASON
//
// A blind critic reported that "from the shoulders up, a kick strip and a run
// strip look nearly identical" and attributed it to a frozen off-hand. The
// attribution was wrong and the observation was correct, and separating the two
