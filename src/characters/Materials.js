 ...
 *
 * REFERENCE ROUND: the SPREAD was right and the LEVEL was a stop and a half too
 * matte. Every one of the eight sheets shows reflected environment on the large
 * plates — paladin's ivory shoulder and volt-monk's chest both carry a legible
 * image of the room, not merely a highlight. 0.57 on the secondary role is what
 * made the navy underlayer in `03-full-body.jpg` read as unfired clay. The three
 * roles keep their ratio (roughly 1.6 : 2.2 : 1) and all three come down, which
 * is only safe because the clearcoat below is allowed to work in the same pass:
 * a satin base under a real coat is glossy, a satin base under a coat that has
 * been multiplied down three times is mud.
 *
 * REFERENCE ROUND 2: down another third. The five sheets with clean paint
 * (`paladin`, `volt-monk`, `vesper`, `neon-ronin`, `ghostframe`) all show the
 * key as a HARD-EDGED streak on a shoulder or a thigh — a lobe narrow enough
 * that the highlight has a shape, not a soft gradient across the whole plate.
 * A GGX lobe reads as "shaped" below roughness ~0.30 and as "diffuse bloom"
 * above ~0.45, and the map above multiplies this by the material's own scalar
 * plus up to +0.25 of structure, so the base has to sit near the bottom of that
 * window for the delivered value to land inside it at all. The ratio between
 * the three roles (roughly 1.6 : 2.2 : 1) is unchanged — it is the part of this
 * constant that was never wrong.
 */
const ROLE_ROUGH = [0.21, 0.29, 0.13];
 ...
 *
 * DENSITY. `minCell 0.055 / depth 7` produced 66 rects per tile. That is
 * sheet-metal wallpaper: count the splits on any plate in the eight reference
 * sheets and it is two to four, deliberate, following the form. An armour plate
 * samples a patch of this atlas roughly a sixth of it across, so 66 rects put
 * four to eight splits on every plate on every fighter. 0.09 / 6 lands at 36,
 * which is half that — and the analytic per-plate lattice, capped at two
 * divisions per axis this round instead of four, supplies the splits that are
 * actually meant to follow the form.
 *
 * THE SMALL-CELL THRESHOLDS MOVE WITH IT, and they have to. Accent paint,
 * louvres and light strips are all gated on `area < 0.016`, which was five
 * times the smallest cell the old layout could produce and is BELOW the
 * smallest cell this one can: left alone it silently deletes the accent role,
 * every vent and every emissive strip from the whole roster, which was measured
 * on the first attempt at this change (0 accent, 0 vent, 0 glow out of 15).
 * 0.035 / 0.016 keeps the same fraction of plates in each role as before.
 */
// AND THEY MOVE AGAIN WITH IT, for exactly the reason the paragraph above
// records. At minCell 0.14 the SMALLEST cell the layout can emit is 0.0196, so
// a `tiny` threshold of 0.016 is unreachable and a `small` threshold of 0.035
// catches only a handful of the ~16 rects — which silently starves the accent,
// vent and glow roles the same way the first attempt at the density change did.
// 0.055 / 0.024 keeps roughly the same fraction of plates in each role over the
// coarser layout: ~40% small, ~10% tiny.
const PANEL_SMALL = 0.055;
const PANEL_TINY = 0.024;

// 0.09/6 landed at 36 rects and the reference round still measured "a uniform
// rectangular tiling" on ANVIL's shoulder in-game. The arithmetic that predicts
// it: an armour plate samples about a sixth of the atlas across, so 36 rects is
// still 3-6 splits on every plate, and the analytic per-plate lattice adds up to
// two more per axis on top. 0.14/5 lands at ~16 rects, i.e. 1-2 atlas splits per
// plate, which leaves the lattice as the thing that draws the deliberate ones.
function buildPanelLayout(rng, { minCell = 0.14, depth = 5, stopChance = 0.14 } = {}) {
  const rects = [];
  ...
  const emit = (x0, y0, x1, y1) => {
    ...
    const z = rng.next();
    const small = area < PANEL_SMALL;
    const tiny = area < PANEL_TINY;
    rects.push({
      x0, y0, x1, y1, area,
      paint: small && h > 0.86 ? PAINT_ACCENT : h < 0.3 ? PAINT_SECONDARY : PAINT_PRIMARY,
      variation,
      z,
      // 0.72 -> 0.86 and 0.5 -> 0.34. Count the louvres and the rivet rows on
      // any of the eight sheets: `paladin` has ZERO of either on its ivory
      // shells, `volt-monk` zero, `vesper` zero; `atlas-7` and `furnace` are the
      // two that rivet, and even they fasten a plate's perimeter, not every
      // plate's perimeter. At the old thresholds roughly one small plate in four
      // was a vent and half of every plate carried a rivet march, which at
      // atlas scale put a fastener row on every armour panel in the cast — the
      // "dense pepper mottle over every plate face" the reference round was
      // handed. Both are now a minority feature.
      vent: small && !tiny && variation > 0.86,
      rivets: z < 0.34 && !tiny,
      bead: z > 0.94,
      ...
      // 0.14 -> 0.05. §1.6 wants ONE hero emitter per fighter plus thin groove
      // light; a lit seam on one small plate in seven is a constellation, not a
      // hero, and it is what put "roughly forty fat mint-green bars" across
      // AXIOM. At 0.05 a fighter's plate atlas carries one or two lit seams.
      glow: small && variation < 0.05 && x0 > 0 && y0 > 0 && x1 < 1 && y1 < 1,
    });
  };
 ...
 *
 * --- REFERENCE ROUND: 0.006 at aspect 3 is half of the "weave" ---------------
 *
 * The eight character sheets are clean automotive paint. Every one of them shows
 * a large plate with NOTHING on its face between the panel splits — the surface
 * incident lives in the clearcoat highlight and at the edges, not in a field of
 * tooth. `docs/shots/03-full-body.jpg` shows the opposite: a directional corduroy
 * over every plate on both fighters, and this constant supplies one of its two
 * strands. `aspect: 3` is what makes it directional, and because armour plates
 * carry per-plate box UVs the lay lands at a different angle on every plate, so
 * forty plates read as forty scraps of woven cloth rather than as one machine.
 *
 * Height halves and the aspect goes nearly round: what survives is a fine
 * isotropic cast tooth that still keeps the plate off "smooth and empty" (the
 * 0-amplitude column above) without giving the eye a direction to latch onto.
 * `rough` is untouched — a roughness grain at this scale is invisible as pattern
 * and is the half of the term that was never the problem.
 */
// `aspect` 1.6 -> 1.0. The note above argues a directional lay reads as "a
// rolled-and-painted plate", and it does — at aspect 3-5, on bare stock. On a
// clearcoated shell it is a second parallel pattern crossing the machining lay,
// and this round's brief is that NO repeating micro-pattern may survive on a
// primary plate at any camera distance. Isotropic at 2-4 texels is the one
// thing that cannot read as a stripe: it is what a blasted or cast surface is,
// and it is the only near-Nyquist term left in the bake. `rough` comes down
// with it because the grain is the loudest single roughness octave in the map
// (the note at PLATE_ANISOTROPY measured it as the only one with travel) and at
// 0.075 it is the pepper the reference round found on VULKAN's chest.
const GRAIN = { rough: 0.042, height: 0.003, aspect: 1.0 };
...
function buildPlateDetail(size) {
  ...
  const n = size * size;
  // Groove half-width in texels. Was 0.0036 — about 3.7 texels at 1024, so a
  // panel split rendered as a 7-8 texel dark band. The reference's splits are
  // hairlines: a shut line on a car body is 4mm over a 2m panel, and at this
  // atlas scale that is under three texels total. 0.0026 halves the band and is
  // the second half of "thin, dark, sparse" (the first is the layout above).
  const gw = Math.max(2, size * 0.0026);
  ...
  // 2.6 per texel of width is 2662 strokes over a 1024 tile — one every 20
  // texels in both axes, which is not scuffing, it is a stipple, and it arrives
  // in albedo (V_CUT, i.e. WHITE), in roughness and in the normal at once. That
  // is the "dense speckled pepper mottle over every plate face" the reference
  // round measured on VULKAN's chest. §1.6 allows "a couple of chips" and a
  // bright line on a leading edge. 0.55 is ~560 strokes, sparse enough that a
  // 200px plate crop shows a handful rather than a field.
  //
  // ROUND 5: 0.55 IS STILL THE SPECKLE, AND IT IS PHOTOGRAPHED.
  // shots/r4/pair1-anvil-head.png at 4x shows ANVIL's yellow pauldron carpeted
  // in short dark COMMAS — thirty-odd per plate — and the same marks are on
  // NYX's helm dome (pair3-nyx-head) and BASTION's chest (pair3-bastion-body).
  // They are these strokes: `curl: 0.42` over an 8-60 texel length is a comma by
  // construction, and each one writes V_CUT into the albedo, 0.14 into
  // roughness, 0.85 into metalness and a notch into the height at once, so it
  // is four different kinds of loud at 1px. The arithmetic that was missed the
  // first time is the SAMPLING: a plate's box UVs cover roughly a sixth of the
  // atlas per axis, so a single pauldron sees ~1/36 of the tile — 560 strokes
  // over 1024^2 is ~15 strokes on that one plate, which is the field in the
  // capture, not the "handful" the note above predicted.
  //
  // 0.05 is ~51 strokes on the whole tile, i.e. one or two on a plate: §1.6's
  // "a couple of chips". Strength comes down with it because at this density
  // each surviving stroke is read individually and a 0.42 stroke on a clean
  ...
  for (let i = 0; i < n; i++) {
    ...
    height[i] =
      ...
      beadH +
      macro[i] * 0.05 +
      // 0.022 / 0.012 -> 0.013 / 0.005. `casting` (fbm freq 12) and `tooth`
      // (freq 64) are the two remaining FULL-FIELD octaves in the plate height,
      // and with the machining lay already gone from this loop they are what is
      // left drawing an all-over pattern into the normal. Freq 64 over a 1024
      // bake is a 16-texel cell; a helm dome samples enough of the atlas to land
      // that at 8-12 screen pixels at closeup, which is exactly the dimpled
      // "hammered" read on NYX's helm in shots/r4/pair3-nyx-head.png and the
      // granular field on BASTION's thigh in pair3-bastion-body.png. §1.6 wants
      // NOTHING on a primary plate's face between the panel splits.
      //
      // Not deleted — the 0-amplitude column in the {@link GRAIN} note is the
      // record of what a perfectly flat plate looks like, and it is worse. At
      ...
      // but on a painted and clearcoated panel they are under the paint film and
      // the eye should read them as a faint sheen break, not as corrugation.
      //
      // 0.004 was "most of the way out" and the reference round measured the
      // corduroy STILL VISIBLE WITHOUT MAGNIFICATION on ANVIL's pauldrons at
      // ~2px pitch. There is no amount of an aspect-14 field in a normal map
      // that is not a repeating stripe: freq 180 over a 1024 bake is a 5.7-texel
      // period, which lands at 5-7 screen pixels on a plate at closeup — dead in
      // the band the eye reads as pattern. It is gone from the normal outright.
      // The energy is not lost: it stays in roughness below (at a third of its
      // old weight), where a directional micro-lay belongs and where it can only
      // ever break a highlight rather than draw a line.
      microScratch[i] * 0.05 -
      heroScratch[i] * 0.1;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    //
    // REFERENCE ROUND. Both halves of this max() are wear, and the reference
    // only wants one of them. `furnace` and `atlas-7` are the two dirty sheets
    // in the set and even they carry bare metal as a LINE on a leading edge plus
    // a handful of chips — never as the field of pale blotches this file's flake
    // term lays across open plate faces. So the flake keeps its outline and its
    // curvature gate but is cut to a sixth of its weight: enough that a corner
    // still shows the odd break, not enough to compete with the lip.
    //
    // The rim window narrows with it. At (0.05,0.26)/(0.36,0.66) of the bevel
    // ramp it was roughly twelve texels of bare metal, which at plate scale is a
  ...
  for (let i = 0; i < n; i++) {
    ...
    // `blot` is a full-field ridged fbm and it used to arrive through a
    // `0.4 + cavity` gate — a 0.4 FLOOR, i.e. oxidation mottle at nearly half
    // strength over open, unbroken plate face. That is the one term in this
    // composition with no physical claim on where it lands, and it is most of
    // why the plates in `03-full-body.jpg` look dipped rather than painted.
    // Multiplying by cavity outright puts it where oxide actually blooms: the
    // bottom of a trough, the shadow of a step, the lee of a rivet.
    // The other three are already positional — cavity, the panel groove itself,
    // and drips that start at a louvre lip — and they are left alone.
    grime[i] = clamp01(cavity * 0.68 + groove[i] * 0.5 + drips[i] * 0.95 + blot * 0.30 * cavity);
  ...
  for (let i = 0; i < n; i++) {
    ...
    // The four structure terms summed to about +0.25 of mean roughness on top
    // of the role base, which is the difference between "satin paint with a
    // gloss break in it" and "everything is 0.6". They keep their relative
    // weights — the gloss break across a panel is the point of the block — at
    // roughly two thirds of the amplitude, so the base can come down without the
    ...
    // 0.5 -> 0.16, and this is the "chalky-matte, zero tight specular" half of
    // the round-4 verdict. `d` is `up * 1.9 * (...) * ao` — UP-FACING texels —
    // and on a plate atlas something like a third of the tile faces up, with the
    // 1.9 gain saturating most of that third. At 0.5 a third of every painted
    // plate was being dragged HALFWAY TO 0.93 roughness before a single grain of
    // dirt was placed, and the plates it hits hardest are the ones whose normals
    // point at the key: pauldron tops, helm crowns, the collar deck. That is
    // precisely where a clearcoat highlight is supposed to live, and it is why
  ...
  for (let i = 0; i < n; i++) {
    ...
    const cav = clamp01((1 - ao[i]) * 1.4);
    let rough =
      0.26 + (patch[i] * 0.13 + casting[i] * 0.12 + tooth[i] * 0.07 + machining[i] * 0.02) * AB.roughStruct +
  ...
  //
  // --- REFERENCE ROUND: the satin verdict above is right for a scrapyard and
  // wrong for these eight sheets ----------------------------------------------
  //
  // The note above is a correct account of a real overshoot — 63.7% of the map
  // under 0.10 was a show-car finish — and the correction it made was the only
  // one available at the time, because the shader was multiplying this coat down
  // twice more downstream (see STORY_CLEARCOAT_FRAGMENT). Three multiplicative
  // attacks on one coat is why NO plate in `03-full-body.jpg` carries a tight
  // highlight, and the contract's §1.6 is unambiguous: "crisp, tight specular
  // highlights and visible reflected environment on the large plates".
  //
  // So the fix is applied at all three places at once rather than here alone.
  // This one authors a coat that is mostly present and mostly smooth; the
  // shader's patchiness terms are halved rather than removed, so the coat still
  // varies — a dulled patch beside a glossy one — but it varies between 0.06 and
  // 0.2 instead of between 0.15 and 1.0. The dirt terms keep the LARGEST share
  // of what is left (grime 0.22 against patch 0.05), which is what keeps the two
  // dirty fighters' crevices from reading as wet lacquer.
  const ccPx = new Uint8Array(n * 4);
  for (let i = 0; i < n; i++) {
    const film = clamp01(0.5 + macro[i] * 0.5 + casting[i] * 0.4 + patch[i] * 0.55);
    const strength = clamp01(
      (0.80 + film * 0.30) * (1 - chip[i]) * (1 - scratch[i] * 0.9) * (1 - grime[i] * 0.55),
    );
    // `dust` 0.15 -> 0.05, for the same reason it came off the base roughness
    // above: it is an up-facing mask that saturates over a third of the tile, so
    // as a coat term it was not "grit dulls a horizontal lip", it was a second
    // matte wash laid over every surface that faces the key. The shader adds its
  ...
  // The strip half rides the groove mask, so it narrowed with `gw` above: a lit
  // seam is now about five texels across where it was eight, which is the
  // "emissive is thin and linear" half of contract §1.6. The squaring is what
  // keeps the falloff inside that width rather than blooming across the plate,
  // and the vent half is deliberately left broad — `furnace`'s glowing rib vents
  // ARE wide bands, and they are the one hero element on that sheet.
...
// Raised from 0.05. At that value a panel split converged on very nearly black,
// so a plate boundary read as a HOLE punched through the armour rather than as a
// shut line with structure at the bottom of it. Every reference sheet shows the
// same thing at a split: a dark line whose floor is still a lit surface, one or
// two stops under the plate beside it, never a void. 0.14 is that.
const V_STRUCTURE = 0.14;            // unpainted frame seen down a panel gap
 ...
 * @param {number} gain      overall reflectance multiplier, 1 = paint
 *
 * `gain` exists for exactly one consumer and it is not cosmetic. This map is a
 * reflectance MODULATION over `material.color`, and on a painted plate that is
 * correct: paint has a diffuse albedo and a role-driven value break is what
 * makes the panel layout read. `kb.worn` is not paint any more — it is the
 * bright-work (see the note on that material), so its `color` is a conductor's
 * F0 and this map multiplies it. At gain 1 the mean of this composition is
 * about 0.5, which halves every F0 in the roster: NYX's textbook gold
 * (1.00, 0.73, 0.34) arrives at the shader as (0.5, 0.36, 0.17), which is not
 * gold, it is a dark brown dielectric-looking thing — the "matte brown dome"
 * the capture round found at her knee. 1.6 puts the mean near 0.8 while the
 * panel gaps and the fastener structure still sit visibly below it.
 */
function composePlateAlbedo(detail, wear, wearFloor = 0, gain = 1) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const t3 = pid * 3;
    const role = t3 < 1 ? 0 : t3 < 2 ? 1 : 2;
    // The `mottle` factor is a full-field casting/macro fbm — by construction
    // all-over noise on the largest surface in the game, which §1.6 forbids on
    // the primary plates in as many words. It is not deleted: a plate with a
    // perfectly constant albedo reads as vector art. At +-9% instead of +-18%
    // it is a batch variation the eye finds only when it looks for it, rather
    // than the "dense pepper mottle over every plate face" the capture round
    // measured on VULKAN's chest.
    //
    // +-9% -> +-5%. It is still the same term and still the same argument; the
    // round-4 capture just says the number has not arrived yet. `mottle` is
    // `0.5 + casting * 0.42 + macro * 0.3` — freq 12 and freq 4 — and the
    // decimetre half of that is what reads as the "mottled blue" over BASTION's
    // thigh and chest in shots/r4/pair3-bastion-body.png. A sprayed batch does
    // vary; it does not vary by a fifth of a stop across one panel.
    let v = lerp(0.65, V_PAINT[role], AB.panelValue ?? 1) * (0.87 + (t3 - role) * 0.28) * (0.95 + mottle * 0.10);
    ...
    // Grime at 0.9 made dirt a COLOUR event — a soot-toned smear painted into
    // the diffuse. On the two dirty sheets (`furnace`, `atlas-7`) grime is
    // overwhelmingly a gloss event: the crevices are the same iron as the plate,
    // they are simply dead where the plate is wet. Halving the albedo coupling
    // leaves the roughness and clearcoat coupling (which is where dirt belongs)
    // at full strength and stops the whole cast reading as though it had been
    // dipped in a bucket.
    v = lerp(v, v * 0.25 + V_GRIME, clamp01(grm * 0.55));
    // 0.6 -> 0.22. `dst` is up-facing texels times a casting field times AO —
    // positional, but not SMALL: on a plate atlas roughly a third of the tile
    // faces up, so at 0.6 a third of every plate was being lifted toward 0.86
    // reflectance in pale grit. That is the light worm-shaped patching visible
    // across the collar plate in the first capture of this round. Settled dust
    // is real and belongs on a horizontal lip; it is not a third of the paint.
    // 0.22 -> 0.10, following the same mask off the roughness and the coat (see
    // the painted ORM loop): the pale worm-shaped patching is still legible on
    // ANVIL's collar deck in shots/r4/pair1-anvil-head.png, and V_DUST is 0.86
    // reflectance — on a dark fighter it is the single brightest thing this
    // function can write outside a chip.
    v = lerp(v, V_DUST, dst * 0.10);

    // A touch of the baked cavity term in the diffuse keeps plates reading as
    // separate objects even under flat fill light.
    const b = encodeSrgb(clamp01(v * gain) * (0.55 + ao * 0.45) + dither(i));
    px[o] = b;
    px[o + 1] = b;
    px[o + 2] = b;
...
function getShared(sizes, maxAniso) {
  ...
  s = {
    ...
    // `wearFloor` is baseline wear applied to EVERY texel by construction, so it
    // is the one parameter in this file that cannot produce edge-only damage: at
    // 0.06 the primary armour of every fighter in the cast carried a uniform
    // 6% of stripped paint before a single edge was consulted. It goes to zero
    // on the clean paints, and the wear multiplier comes down with it — the
    // bright metal line the reference actually shows is drawn analytically by
    // kbLip and the specular chamfer, not by this mask.
    //
    // `worn` USED to keep a real floor because it was the stripped-plate
    // material. It is not any more — see the note on `kb.worn` in
    // `makeMaterialLibrary`: RobotBuilder builds both the polished bright-work
    // and the accent panels out of it, and neither of those is a stripped
    // plate. A 0.18 wear floor is 18% of bare steel on every bezel ring and
    // accent panel in the cast BY CONSTRUCTION, before an edge is consulted,
    // which is the "flat mustard" and "chalky salmon" the reference round
    // measured on trim. 0.6/0.02 is a used but painted-and-plated surface.
    plateAlbedo: t(composePlateAlbedo(plate, 0.7, 0), plate.size, 1, { srgb: true }),
    // The accent panel is the best-finished paint on the machine and the one the
    // eye is meant to go to, so it is also the least worn of the three.
    plateAlbedoTrim: t(composePlateAlbedo(plate, 0.5), plate.size, 2, { srgb: true }),
    plateAlbedoWorn: t(composePlateAlbedo(plate, 0.6, 0.02, 1.6), plate.size, 2, { srgb: true }),
...
//
// REFERENCE ROUND: both swings come down again. This term is UNGATED — it runs
// over every texel of every plate at the grunge tile's 10-30cm wavelength — so
// whatever it is set to is, by definition, all-over noise, and the contract's
// §1.6 asks for none of that on the primary plates. What a fading topcoat
// really does at that scale is +-7% of value and a modest gloss break, which is
// what is left here. The gloss half stays the larger of the two for the reason
// the note above gives; it is simply no longer half a roughness unit wide.
diffuseColor.rgb *= mix( 1.0, 0.92 + 0.14 * kbG.r, kbStory.z );
roughnessFactor = clamp( roughnessFactor + ( 0.52 - kbG.r ) * 0.34 * kbStory.z, 0.04, 1.0 );
...
//
// That is what the comment said and the arithmetic did not: 0.35 + 1.3*kbEdge
// is a FLOOR: with kbEdge at zero — mid-plate, as far from a gap as a texel can
// be — oxide still fired at 35% of its gated strength, which is exactly the
// smeared oxide over open faces the contract names. 0.05 leaves a trace so the
// transition is not a hard cut and hands the rest of the range to the gate.
float kbOx = smoothstep( 0.26, 0.78, kbG.b * ( 0.05 + 1.35 * kbEdge ) ) * kbStory.y;
...
//
// Same floor, same consequence as the oxide above, and worse here because
// kbSide is 1 - abs( n.y ), which is ~1 over almost the whole of a standing
// figure: the run term was kbG.g * 1.0 * 0.45 — nearly half strength — wherever
// the surface was not horizontal. Dropping the floor to 0.05 leaves the run on
// the panel-gap field it was always documented to be gated on, and leaves
// kbCavity (the baked crevice mask) as the term that carries dirt into the
// crevices, which is where the reference keeps it.
float kbRun = kbG.g * kbSide * ( 0.05 + 1.2 * kbEdge );
...
if ( kbDetail.x > 0.0 ) {
	...
	//
	// REFERENCE ROUND: the chatter term is what turned a lay into a PLAID.
	//
	// kbSlope is a carrier ACROSS the lay axis. kbChat modulates its amplitude
	// ALONG the same axis at ~30mm. An across-carrier times an along-modulator is
	// a cross-hatch by construction, and because the axis is per-plate-hashed and
	// skewed by up to +-8.5 degrees it lands DIAGONAL on most plates — which is
	// precisely the "all-over diagonal hatch" covering both fighters in
	// docs/shots/03-full-body.jpg, and the first item this round was given.
	//
	// Machining chatter is real, but on a plate that has since been primed,
	// sprayed and clearcoated it is under a film and is not a second pattern
	// crossing the first. Taking the swing from +-0.38 to +-0.10 leaves the mean
	// nearly unchanged and removes the cross-modulation, so what survives is a
	// single-direction lay — which is what STORY_DEFAULTS.detail then holds at a
	// quarter of its old strength for the painted plates.
	float kbWF = 6.2831853 / kbLamF;
	float kbWC = 6.2831853 / kbLamC;
	float kbChat = 0.90 + 0.10 * sin( kbAlong * ( 2.3271 / kbLamC ) + kbPhase );
...
// This is the reference's edge wear, and once the flake half of the baked chip
// mask is gone (see the chip loop in buildPlateDetail) it is carrying the read
// on its own — so it is pushed up rather than left where it was. It is the right
// term to spend on: it is a LINE on a free ground edge, it is analytic so it
// band-limits itself, and it costs nothing per plate.
diffuseColor.rgb = mix( diffuseColor.rgb, kbSteelColor * ( 0.34 + 1.25 * kbLipLum ), kbLip * 0.74 * ( 1.0 - kbGrime * 0.6 ) );
roughnessFactor = clamp( roughnessFactor + 0.18 * kbJoint + 0.06 * kbHaloJ - 0.34 * kbLip, 0.06, 1.0 );
metalnessFactor = mix( metalnessFactor, 1.0, kbLip * 0.68 );
...
if ( kbLattice.x > 0.0 && kbPitch > 0.0 && vKbFrame.z > 0.0 && vKbFrame.w > 0.0 ) {
	...
	// Cap: was 4, i.e. up to sixteen panels on one plate face. Count the splits
	// on any plate in the eight reference sheets and it is one or two per axis —
	// a deliberate division, not a grid. 2 is that, and it also halves the number
	// of dark lines competing with the plate's own perimeter, which is the line
	// the eye is meant to read.
	vec2 kbCells = min( floor( kbSpan / ( kbPitch * kbLattice.w ) ), vec2( 2.0 ) );
	...
	// The gap darkens; the plate BESIDE it does not. 0.58/0.17 took the split to
	// near-black and then shadowed a band either side of it, so a divided plate
	// read as two plates with a trench between them. The reference's splits are
	// thin and dark and the paint runs right up to them.
	diffuseColor.rgb *= 1.0 - 0.42 * kbLat - 0.10 * kbLatH;
...
const STORY_CLEARCOAT_FRAGMENT = /* glsl */`
	...
	//
	// This is the THIRD place one coat is attacked — the bake authors it, the
	// map is bound as both clearcoatMap and clearcoatRoughnessMap, and then this
	// runs. Three multiplicative knock-downs is why no plate on the roster
	// carried a tight highlight, and it is the mechanism behind contract item 6,
	// "large plates show a clean clearcoat highlight". The patchiness survives —
	// it is the half of this block that is doing real work — at roughly a third
	// of its old authority, so a dulled patch still sits beside a glossy one but
	// the glossy one is now genuinely glossy. The dirt coefficients are halved
	// rather than removed: dirt killing lacquer is correct, and after the gating
	// fixes upstream kbGrime and kbOx only reach crevices anyway.
	material.clearcoat = saturate( material.clearcoat * ( 0.72 + 0.34 * kbG.r ) * ( 1.0 - kbGrime * 0.55 ) * ( 1.0 - kbOx * 0.8 ) * ( 1.0 - kbMark * 0.4 ) );
	material.clearcoatRoughness = clamp( material.clearcoatRoughness + ( 0.55 - kbG.r ) * 0.18 + kbGrime * 0.18 + kbOx * 0.25 + kbDust * 0.15, 0.0525, 1.0 );
...
const STORY_DEFAULTS = {
  ...
  // `latticePanel` 1.0 -> 1.75. The shader takes
  // `min( floor( span / ( pitch * latticePanel ) ), 2 )` cells per axis, so on
  // any plate wider than twice its pitch it saturates at 2x2 — a nine-rect grid
  // on EVERY plate, which is the "regular quadrilateral tiling" and the
  // "evenly-spaced horizontal ladder" the reference round found on ANVIL and
  // BASTION. Stretching the pitch by 1.75 drops most plates to 1 or 2 cells on
  // their long axis and 0 or 1 on their short one, which is §1.6's "a handful
  // of deliberate splits per plate, following the form" — the plate's own frame
  // is what the split is measured in, so the splits still run with the form.
  // The master comes off 1 as well: at 1.0 the gap core is the darkest thing on
  // the fighter and the note above records that 1.0 is also the ceiling before
  // the halo's sign inverts, so there is no headroom on that side and no reason
  // to sit on the limit.
  lattice: 0.8,
  latticeGap: 0.0045,
  ...
  //
  // REFERENCE ROUND. `detail` reaches exactly two materials — `armor` and
  // `trim`, the two painted plate materials — because every other Story material
  // in the library overrides it. Those two are the ones the contract's item A
  // names: "kill the all-over diagonal hatch on the primary armour materials, it
  // is the loudest wrong thing in 03-full-body.jpg". A quarter of the old
  // strength, over a lay whose cross-modulation has been removed at kbChat, is a
  // faint single-direction sheen break instead of corduroy. It is NOT taken to
  // zero: the roughness handoff at the bottom of that block is scaled by this
  // same uniform and is what keeps the plates from going glassy at range.
  //
  // `detailFine` follows it down from 3.0mm to 1.8mm for a different reason —
  // 1.8mm is about two screen pixels at fight framing, so the surviving octave
  // now falls under its own Nyquist fade at the distance the game is actually
  // played at, and its energy is handed to roughness rather than drawn.
  // REFERENCE ROUND 2: ZERO, and the default is what changes so that every
  // material that does not opt back in is clean. 0.25 was "a faint
  // single-direction sheen break instead of corduroy" and the capture round
  // that followed it measured a "continuous parallel-line corduroy at ~2px
  // pitch" on ANVIL's pauldron, "diagonal on the left plate and vertical on the
  // right" — i.e. exactly this term, whose axis is a per-plate hash. There is no
  // strength at which a full-coverage parallel carrier is not a repeating
  // pattern; retuning it is what the last two rounds did. It is off on painted
  // armour and stays on, weakly, only where the surface physically IS bare
  // machined stock (`darkMetal`, `piston`) and the marks are the material.
  //
  // The roughness handoff at the bottom of that block is scaled by this same
  // uniform, so taking it to 0 also removes the +0.10 of range-roughness it was
  // adding to every painted plate. That is wanted here: the plates are meant to
  // stay glossy at range, and ROLE_ROUGH now carries the base.
  detail: 0,
  detailFine: 0.0018,
  detailCoarse: 0.011,
  detailWear: 0.8,
  ...
  // REFERENCE ROUND: narrowed and brightened. 2.8mm is a five-pixel band at
  // closeup, which reads as a lit STRIP; the reference's edge break is a line.
  // 2.1mm is three pixels, and `chamferGloss` above 1 saturates the clamp on the
  // core of the facet sooner, so the middle of the break goes to bare polished
  // alloy while the ramp either side of it still falls off. That pair — thinner,
  // harder — is the contract's "thin bright line of exposed polished metal on
  // leading edges", and with the flake wear gone it is the whole of item C.
  chamfer: 1,
  chamferWidth: 0.0021,
  chamferSlope: 0.84,
  chamferGloss: 1.25,
 ...
 * @property {THREE.MeshPhysicalMaterial} chrome     polished bright-work in `palette.trim` — rings, bezels, rims, fasteners
 ...
 * @property {THREE.MeshPhysicalMaterial} underskin  dark ribbed mechanism under the armour — neck, waist, inner joints
...
export function makeMaterialLibrary(renderer, palette = DEFAULT_PALETTE, options = {}) {
  ...
  // THE DARK SECOND BODY, §1.3. `kb.darkMetal` is what RobotBuilder actually
  // builds the exposed mechanism out of — the abdominal ring stack, the limb
  // ribs, the joint hubs, the frame between the shells — 44 call sites. The
  // contract asks that layer to read DARKER than the armour over it ("matte
  // charcoal/graphite ... the eye sees armour, then a gap, then mechanism") and
  // the reference round measured the opposite: "the underskin that does exist is
  // the wrong value — hot pink on Vulkan, polished chrome-blue on Volta,
  // brighter than the armour it is supposed to hide behind, so you read a
  // glowing spring rather than shadow and machinery."
  //
  // 0.115 is not a dark F0. It is roughly cast iron, and a conductor at that F0
  // with anisotropy 0.62 and envMapIntensity 1.05 mirrors the arena's practicals
  // straight back at the camera — which on a stack of a dozen 3mm rings is a
  // ladder of specular highlights, i.e. the "glowing spring". 0.072 is graphite
  // and it is the darkest F0 in the library that is still a metal rather than a
  // hole.
  //
  // The HUE amount comes down with it, 0.5 -> 0.22. §1.3 does not ask for a
  // second body in the character's colour, it asks for "a matte
  // charcoal/graphite layer" — the same substance on all eight sheets, because
  // it is the machine under the paint rather than part of the paint scheme. At
  // 0.5 of `secondary` this took half its hue from the palette, which on
  // BASTION's navy gave it a stack of BLUE waist rings that answered the warm
  ...
  // nothing except a `sheenColor` on the cable loom. Every reference sheet uses
  // that metal as its SECOND colour — vesper's gold ring bezels, volt-monk's
  // brass spine, atlas-7's brass hubs — so this material now takes its hue, and
  // takes far more of it: at 0.13 a brass and a nickel are the same grey.
  // `alloy` renormalises to the neutral's luma, so a dark trim entry still comes
  // out as a bright metal; only the hue crosses over.
  //
  // 0.62 rather than something safer, and clamped rather than left alone,
  // because a conductor's F0 is a SATURATED number and this is the one material
  // where that matters. At 0.38 the brass entries came back at (0.94, 0.73,
  // 0.56) — a pale champagne — where real polished brass is (0.91, 0.75, 0.35)
  // and gold is (1.00, 0.71, 0.29). At 0.62 NYX's gold lands on (1.00, 0.73,
  // 0.34), i.e. the textbook value, and the nickel entries land on a neutral
  // (0.74, 0.77, 0.83) because a desaturated `trim` has nothing to push with.
  // The clamp is what makes 0.62 safe: renormalising a dark saturated hue to the
  // neutral's luma can drive one channel past 1, and a reflectance over unity is
  // energy the surface does not have.
  const trimMetal = hexToLinear(p.trim);
  /**
   * How POLISHED this character's bright-work is, derived from the reflectance
   * of its own `trim` entry rather than authored a second time in the roster.
   *
   * The roster has spent three rounds darkening `trim` on the dark-hardware
   * fighters and the captures keep coming back with the same fault: RONIN-07 at
   * a #545C6A F0 — a genuinely dark, genuinely COOL grey — photographs as a
   * gold samurai (shots/r4/pair2-ronin-body.png), and NYX's torso, arms and
   * thighs photograph gold-bronze against a sheet that is void black
   * (pair3-nyx-body.png). Darkening the F0 alone cannot fix it, because a
   * conductor has no colour of its own: what you see is F0 times the
   * environment, this arena's environment is a warm key over sodium practicals,
   * and `kb.worn` was reflecting it at roughness 0.13-0.25 with
   * envMapIntensity 0.95 — a MIRROR — across the 77 builder call sites the
   * `trim` batch covers. A mirror of a warm room is a warm mirror at any F0.
   *
   * The missing half is that darkness and polish are not independent on real
   * hardware. Blackened iron, blued steel, black-oxide conversion coating and
   * antique gold are all dark BECAUSE they are microscopically rough conversion
   * layers; polished brass, gold and nickel are bright because they are not.
   * So one number does both: a bright `trim` entry keeps the mirror ANVIL's
   * brass hubs and VOLTA's collars need, and a dark entry gets the satin lobe
   * that a blued or oxidised finish physically has, which is what finally stops
   * the reflection out-competing the paint on the fighters whose sheets have no
   * bright-work at all.
   *
   * The window is set off the entries themselves: the polished half of the cast
   * (ANVIL 0.47, SERAPH 0.41, AXIOM 0.43, KESTREL 0.54, VULKAN 0.31, VOLTA 0.31)
   * all sit above 0.30 and are untouched; BASTION 0.22 is dulled a little, and
   * the three the captures name — RONIN 0.037, MANTIS 0.098, NYX 0.16 — land at
   * or near the oxide end (delivered polish 0.00 / 0.10 / 0.41).
   */
  const trimPolish = smoothstep(0.05, 0.30, lumaOf(trimMetal));
  const chromeAlloy = linearColor(alloy([0.76, 0.765, 0.78], trimMetal, 0.62).map((v) => Math.min(1, v)));
  ...
  const armor = new StoryPhysicalMaterial({
    name: 'kb.armor',
    // Was `story({})`, i.e. every ink at full strength on the material that
    // covers most of every fighter. The gating fixes upstream mean these terms
    // now land in crevices and at panel gaps rather than over open field, so
    // they are worth keeping — but the reference's primary plates are CLEAN
    // painted panels and the right weight for a history layer on them is a
    // fraction, not unity. `kb.worn` keeps the full dose and is where the two
    // dirty sheets (`furnace`, `atlas-7`) get their character from.
    //
    // REFERENCE ROUND 2. Every ink comes down again, because the capture the
    // last set of numbers produced still measured "a dense speckled pepper
    // mottle over every plate face" and "a smeared brown blotch over the centre
    // of the blue centre panel". These four terms are all UNGATED or
    // near-ungated fields sampled triplanar at 10-30cm, so whatever weight they
    // carry is by definition all-over noise on the largest surface in the game,
    // and §1.6 asks the primary plates for none of it. What is left is a trace:
    // enough that two plates on the same fighter are not the identical sprayed
    // panel, not enough to read as an event on any one of them.
    //
    // `marking` was on the STORY_DEFAULTS value of 1 — the stencil layer at FULL
    // strength on the primary armour of all ten fighters. That is the "KB
    // 271-93" spray on ANVIL's shoulder and the smeared blotches elsewhere; two
    // of the eight sheets carry a small stencil and six carry none. `bare` also
    // comes off 0.75: it washes the diffuse toward neutral steel in proportion
    // to metalness, and with the chip mask cut to a sixth it is now doing more
    // desaturating than the wear it exists to describe.
    //
    // `burnish` IS NOW STATED, AND IT IS THE PAINT'S BIGGEST REMAINING LEAK.
    // It was on STORY_DEFAULTS' 0.55 — never chosen for this material, simply
    // inherited — and STORY_FORM_FRAGMENT spends it as
    // `mix( diffuseColor.rgb, kbSteelColor * (0.35 + 1.3 * luma), kbBurn * 0.7 )`
    // plus `mix( metalnessFactor, 1.0, kbBurn * 0.6 )`, gated on kbRoll, which
    // is the GEOMETRIC CURVATURE of the fragment. That gate was written when the
    // fighters were boxes: on a slab only the chamfer is curved, so the term was
    // an edge polish. §1.1 has since turned every limb into a tapered capsule
    // and every plate into a doubly-curved shell — so kbRoll is now nonzero over
    // the WHOLE of a thigh, a forearm and a pauldron, and an edge polish became
    // a flat wash of pale neutral alloy across the fighter.
    //
    // It is invisible on a light palette and it is fatal on a dark one, because
    // it is additive toward 0xc2c6cb: on ANVIL's yellow it is a few percent, on
    // NYX's #12101A void black and RONIN-07's #141418 lacquer it multiplies the
    // delivered value several times over and drags the metalness up with it, and
    // a pale semi-conductor under this arena's warm key is BRONZE. That is the
    // gold-bronze on NYX's thighs and RONIN's forearms in shots/r4, and it is
    // also why both are correct in one body frame and wrong in the other: kbRoll
    // is built from screen-space derivatives divided by the view footprint, so
    // the term is VIEW-DEPENDENT and changes with framing.
    //
    // 0.10 keeps a trace on a genuinely sharp rolled edge — which is where the
    ...
    // Was 1.0. The plate normal carries the panel relief, the rivet crowns and
    // the oil-canning, all of which must survive — but it also carries the
    // residue of the machining/grain weave, and pulling the whole map back a
    // third is the cheapest way to take the last of that off the primary plates
    // without unpicking a bake the rest of the library shares.
    // 0.7 -> 0.45. With the machining lay gone from the bake this map is panel
    // relief, rivet crowns and oil-canning — real form the plate should keep —
    // over a residue of casting mottle that is the last broadband thing in the
    // normal. Pulling it back also widens nothing: a normal map cannot make a
    // highlight tighter, only rougher, so this is the cheapest remaining move
    // toward "a large plate renders as a smooth unbroken surface".
    ...
    // 0.90 -> 0.84 in the same pass that took ROLE_ROUGH down, and for the same
    // reason; the +0.12 secondary offset RobotBuilder applies still lands clear
    // of the clamp at 0.96, so the spread this number exists to unlock survives.
    //
    // REFERENCE ROUND 2: 0.84 -> 0.66. The capture came back with "no specular
    // highlight anywhere — flat, chalky, uniformly lit, reading as raw MDF" on
    // RONIN's chest, and a scalar is the only lever that moves the WHOLE map
    // rather than one octave of it. ROLE_ROUGH now authors 0.13-0.29 and the
    // structure stack adds up to ~0.25, so the delivered base lands around
    // 0.25-0.35 — inside the window where a GGX lobe has a shape. RobotBuilder's
    // armorSecondary offset (+0.12) still clears the clamp at 0.78, so the
    // primary/secondary gloss spread this number exists to unlock is intact.
    roughness: 0.66,
    metalness: 1,
    aoMapIntensity: 1,
    ...
    // Was 0.35. The note at PEEL_REPEAT already establishes that a clearcoat
    // normal cannot buy fine scale here — the coat is a 4% reflector with a
    // broad lobe, so perturbing it SPREADS the highlight. Spreading the highlight
    // is precisely the thing this pass exists to stop, so the peel is held to a
    // trace: enough that the coat is not a perfect optical flat, not enough to
    // widen the reflection of the key.
    clearcoatNormalScale: new THREE.Vector2(0.18, 0.18),
    emissive: emissiveColor,
    emissiveMap: shared.plateEmissive,
    // Raised with the groove narrowing in `buildPlateDetail`: the lit area went
    // down by roughly a third, so the intensity goes up by roughly a half to
    // keep the same amount of light in the frame. Thin and bright is the
    // reference's emissive; broad and dim is the current build's.
    //
    // 3.4 -> 1.5 with the glow-plate share cut from 0.14 to 0.05 in
    // buildPanelLayout. That is the other half of "one hero element per
    // fighter": the grooves that remain are meant to be a hairline of light in
    // a shut line, not a second light source competing with the visor, and at
    // 3.4 each one bloomed to several pixels wide.
    emissiveIntensity: 1.5,
    specularIntensity: 1,
    ior: 1.48,
    // AND IT STAYS AT 1. Raised to 1.15 for one capture and reverted: the
    // arena's fill is a strong cyan-teal off the wet deck, and on a
    // near-dielectric painted plate `envMapIntensity` scales the diffuse
    // irradiance as well as the specular — so on ANVIL's safety yellow the step
    // pushed the plates measurably toward green (a shoulder crop went from
    // R>G to G>R), which §3 gives that fighter no permission for. The tight
  ...
  const trimMat = new StoryPhysicalMaterial({
    // NOTE ON THE NAME: this is the ACCENT PAINT, not the polished bright-work.
    // The reference's trim metal is `kb.chrome`, which now takes `palette.trim`
    // as its hue; this material is the accent-coloured panel — paladin's red
    // stripe, aegis's orange band — and is finished to the same standard as the
    // primary because it came out of the same booth.
    name: 'kb.trim',
    // `burnish` stated for the same reason it is stated on `kb.armor`: an
    ...
    anisotropyRotation: 0,
    clearcoat: 1,
    // An accent panel is the best-finished paint on the machine — it is the one
    // the eye is meant to go to — so it holds the tightest coat of the three
    // painted materials. 0.8 was already the tightest; 0.62 keeps the ordering
    // and moves with the bake, which now authors coat roughness an octave lower.
  ...
  // anything here: it builds BOTH of
  //
  //     armorAccent: tint( pick('worn'), palette.accent )
  //     trim:        tint( pick('worn'), palette.trim, { metalness: 1.0 } )
  //
  // out of this one material, and `trim` is the single most-used name in the
  // builder — 77 call sites against armorPrimary's 34. Every bezel disc, bolt
  // ring, collar, lens surround and brow band on every fighter is this
  // material. Contract §1.4 and §1.6 make that surface polished brass, gold or
  // nickel and call it the design's SECOND COLOUR; `vesper`'s gold joint rings
  // and `atlas-7`'s brass hubs are the most-looked-at things on those sheets.
  //
  // It was an oxidised, pitted, grimed plate. That is why the reference round
  // measured NYX's knee bezel as "a matte brown dome with no highlight, reading
  // as felt or clay" and VOLTA's hero shoulder disc as "chalky salmon". A
  // library that ships `kb.chrome` for exactly this job and is never asked for
  // it is a library with a dead material and a wrong-looking cast.
  //
  // So this becomes the bright-work, and `kb.chrome` stays as the reference
  // definition of one (they are now the same substance authored twice — chrome
  // over the metal detail set, this over the plate set, because a bezel lathe
  // and an accent panel carry different UVs). The cost is that `armorAccent`
  // becomes a lacquered METAL accent rather than flat accent paint. That is the
  // right trade against these eight sheets: the accent areas on `neon-ronin`,
  // `vesper` and `aegis-01` all read as anodised or candy-coated metal, and
  // `armorAccent` covers 23 call sites against `trim`'s 77.
  //
  // `metalness` stays under 1 for one specific reason: the builder's `trim`
  // override sets it to 1.0, and if this were already 1.0 that override would
  // be a no-op and the two batches would render term-for-term identical. 0.9
  // keeps the accent a shade less conductive than the bright-work, which is
  // also physically right for a coated panel next to a bare turned ring.
  const worn = new StoryPhysicalMaterial({
    name: 'kb.worn',
    // Every ink that describes DIRT comes down to a trace and `bare` goes to
    // zero. `bare` is the one that mattered most and it was invisible: it mixes
    // the diffuse toward `kbSteelColor` (0xc2c6cb) in proportion to metalness,
    // and this material's ORM writes metalness ~1 over its whole area, so 0.3
    // was a flat 30% wash of pale neutral grey over every trim ring and accent
    // panel in the cast, on top of `burnish` 0.9 doing the same thing again on
    // every curved edge. That is the mechanism behind BASTION reading as
    // cardboard tan when its palette is gunmetal blue, and KESTREL reading as
    // khaki when its palette is arctic white.
    // `burnish` 0.35 -> 0.12, and it is the same class of mistake `bare` was.
    // STORY_FORM_FRAGMENT builds it from `kbRoll`, the GEOMETRIC curvature of
    // the fragment — "a rolled edge is polished by everything that brushes
    // past". That reasoning holds on a chest deck, where only the chamfer is
    // curved. It does not hold on this material's actual population: the `trim`
    // batch is bezel discs, bolt rings, collars, lens surrounds and lames, i.e.
    // small-radius turned and rolled solids whose curvature is high EVERYWHERE,
    // so kbRoll saturates over the whole part and the term stops being an edge
    // and becomes a flat wash. What it washes with is `kbSteelColor` (0xc2c6cb,
    // a bright neutral) mixed into `diffuseColor`, which on a conductor IS the
    // F0 — so every dark hardware entry in the roster was being lifted a good
    // half of the way to pale grey before the environment was consulted. That is
    // the other half of why RONIN-07 and NYX photograph as bright warm metal
    ...
    // 1.15 -> 0.12, in two steps, and the second step was photographed. The old
    // note argued this material wants to read as pitting; it does not any more,
    // because polishing is what REMOVES surface texture and a bezel ring with a
    // pitted normal cannot show the hard bright rim and dark core §1.4 asks a
    // joint disc for. 0.42 was not enough: the first capture came back with
    // ANVIL's pale-brass collar plate reading as coarse WET SAND across its
    // whole face. The mechanism is WORN_REPEAT — this map is `plateNormal`
    // retiled at 0.5, so the plate bake's `macro` octave (fbm freq 4, amplitude
    // 0.05, by far the largest term in that height field) spans two UV units
    // and lands as decimetre-scale blobs on a part that samples a sixth of the
    // atlas. At 0.12 the panel and fastener relief an accent panel needs
    // survives and the blobs are under the paint film where they belong.
    normalScale: new THREE.Vector2(0.12, 0.12),
    aoMap: shared.plateOrmWornTiled,
    roughnessMap: shared.plateOrmWornTiled,
    metalnessMap: shared.plateOrmWornTiled,
    // A scalar over `plateOrmWornTiled`, which authors ~0.26-0.50 of its own, so
    // the delivered roughness lands at 0.13-0.25 — the band §1.6 quotes for
    // polished trim metal, and narrow enough that a bezel shows a travelling
    // rim rather than a flat tint.
    //
    // Now derived from {@link trimPolish}: 0.5 on a polished entry, 0.72 on an
    // oxide one, so a blued or blackened finish delivers ~0.19-0.36 instead —
    // satin hardware rather than a mirror. This is a per-character number now
    // because the roster's `trim` entries are not one substance: half the cast
    ...
    // the same ceiling `kb.chrome` records at 0.3.
    anisotropy: 0.12,
    anisotropyRotation: 0,
    // A polished conductor reads by what it reflects. This is the number that
    // separates "gold" from "mustard paint" in a frame lit mostly by practicals.
    //
    // 1.7 for one capture, then 1.2, and the reason is worth recording because
    // it is the trap in raising this on a metal. A conductor's reflection is
    // TINTED BY ITS F0: gold multiplies the environment by roughly
    // (1.00, 0.73, 0.34), and this arena's fill is a strong cyan off the wet
    // deck. Gold times cyan is GREEN, and at 1.7 that product was the dominant
    // term on ANVIL's accent shells — photographed at 2x on
    // pair1-anvil-head.png as a saturated emerald gradient across a plate whose
    // palette contains no green at all, which is exactly the §3 violation the
    ...
    //
    // And it is now derived from {@link trimPolish} for the last step of the
    // same argument. The reflection is the WHOLE of a conductor's colour, so on
    // the fighters whose sheets have no bright-work — RONIN-07's black oxide,
    // MANTIS's dark olive gunmetal — the env term is not a highlight sitting on
  ...
  const darkMetal = new StoryPhysicalMaterial({
    ...
    story: story({
      ...
      // few centimetres, so the term never behaved as the edge polish its name
      // and its comment claim. At 0.8 it mixed more than half of `kbSteelColor`
    ...
    // 1.0 -> 0.7: the brush field in this map is the one directional pattern
    // left in the library, and on a 3mm rib it is a corrugation rather than a
    // finish.
    ...
    roughness: 1,
    // 1 -> 0.78, and this is the only lever left that can matte this zone: the
    // roughness scalar is already at its ceiling (`metalOrm` authors 0.22-0.45
    // and a scalar can only multiply DOWN), so a full conductor here is a
    // half-mirror by construction and no amount of darkening the F0 changes the
    // shape of its highlight. Below 1 the surface keeps a dielectric lobe
    // alongside the conductor one — broad, white, weak — which is what a
    // machined graphite composite actually is and what `kb.underskin` in this
    // same file already documents at 0.62 for exactly this zone.
    metalness: 0.78,
    ...
    // 0.62 -> 0.24, 1.05 -> 0.55. Both are reflection levers and both are what
  ...
  const piston = new StoryPhysicalMaterial({
    name: 'kb.piston',
    // `burnish` 0.5 -> 0.22, with `kb.armor` and `kb.darkMetal`. A rod is a
  ...
  //
  // Contract §1.4 and §1.6: every joint on every one of the eight sheets carries
  // a disc or bezel in polished metal, and rings, rims, fasteners and edge
  // breaks are the only places that metal appears. On `vesper`, `atlas-7` and
  // `volt-monk` it is brass or gold and it is the character's second colour. It
  // is the most-looked-at material on those sheets and this library was using it
  // for almost nothing.
  //
  // Three changes make it that metal rather than a slightly shiny grey:
  // `palette.trim` supplies the hue (see `chromeAlloy` above), roughness comes
  // down to where a lobe is narrower than a bezel is wide, and the environment
  // term goes up — a polished ring reads by what it REFLECTS, and against a
  // clearcoated plate at envMapIntensity 1.0 a 1.4 metal is not obviously a
  // different substance.
  //
  // If RobotBuilder's `trim` batch is still `tint(pick('worn'), palette.trim)`,
  // this is the material it wants instead: oxidised plate is the wrong source
  // for polished bright-work, and `pick('chrome')` is a one-word change there.
  const chrome = new StoryPhysicalMaterial({
    name: 'kb.chrome',
    // A polished ring carries no machining lay at all — the lay is what
    // polishing removes — so `detail` is nearly off, where every other metal in
    // the library keeps some.
    story: story({ ...bareSteel, grime: 0.25, dust: 0.1, seam: 0.3, burnish: 0.35, hollow: 0.5, lattice: 0.3, detail: 0.1, detailWear: 0.15 }),
    color: chromeAlloy,
    map: shared.metalMod,
    normalMap: shared.metalNormal,
    ...
    roughness: 0.16,
    metalness: 1,
    // Kept, but low: real brass bezels are turned, so there is a faint circular
    // lay left in them. Above about 0.3 the highlight smears into a streak and
  ...
  // THE DARK UNDERSKIN. Contract §1.3, and the single biggest reason the eight
  // reference sheets read as MACHINES rather than as armour suits: under the
  // painted shells there is a second, darker body — ribbed segments, braided
  // bundles, stacked discs — and it is exposed in the same places on all eight
  // (neck, armpit, waist ring stack, inner elbow, back of knee, inner thigh,
  // behind the ankle, finger and toe joints). The eye sees armour, then a gap,
  // then mechanism, at every joint, which is also what makes the joints look
  // like they can move.
  //
  // Nothing in the library was this. `kb.gasket` is the closest and is wrong in
  ...
  // `ribFreq = max(8, size >> 5)`, and this binds the same three maps the gasket
  // and the cable loom already bind. The normal scale is harder than the
  // gasket's 1.35 because the ribs are the whole read here — with the specular
  // gone flat this surface is relief and nothing else.
  //
  // The story layer is off in every term that describes PAINT. An extruded rib
  // stack has no panel lines, no fastener rows, no machining lay and no stencil:
  // leaving `lattice` and `detail` alive here would print a panel grid across
  ...
  const underskin = new StoryPhysicalMaterial({
    ...
    aoMap: shared.softOrm,
    roughnessMap: shared.softOrm,
    // No metalnessMap, deliberately, where `kb.rubber` binds one: the soft ORM
  ...
  // same broad brushed streak the shoulder behind it already has.
  //
  // THE IRIDESCENCE IS THE GREEN PLATE. 0.32 over a 140-480 nm thickness range
  // sweeps the interference tint across most of the visible spectrum as the
  // surface curves, and on a large part it lands on the green lobe over a wide
  // band of angles — which is the "large saturated iridescent green plate on
  // the right shoulder that appears nowhere in the spec" the reference round
  // found on ANVIL, a fighter whose palette is safety yellow and brass. §3
  // limits every fighter to two colours plus trim and no thin-film term can
  // respect that. 0.07 over a 210-300 nm range is a single narrow warm tint
  // rather than a rainbow: a coating, which is what the comment always claimed.
  const bezel = new THREE.MeshPhysicalMaterial({
    ...
    iridescence: 0.07,
    iridescenceIOR: 1.34,
    iridescenceThicknessRange: [210, 300],
    ior: 1.58,
  ...
  const glass = new THREE.MeshPhysicalMaterial({
    ...
    // Same reason as `kb.bezel`: a visor is a coated optic, not an oil slick,
    // and a full-spectrum thin film is a fourth hue on a two-colour character.
    iridescence: 0.12,
    iridescenceIOR: 1.32,
    iridescenceThicknessRange: [220, 320],
    envMapIntensity: 1.5,
    transparent: false,
  });
  ...
  // WHY THIS ONE BINDS NO emissiveMap, AND WHAT THAT COSTS.
  //
  // `kb.armor` and `kb.trim` emit through `shared.plateEmissive`, so on those two
  // the light lives in the groove and nowhere else — thin and linear, which is
  // what contract §1.6 asks for. This material has no map, so anything handed to
  // it glows edge to edge, and RobotBuilder's `rig.glow()` hands it whole PLATES.
  // That is contract item 8, "emissive is broad panels rather than thin grooves",
  // and the remaining half of the fix is not in this file: it is replacing broad
  // glow faces with `channelStrip` grooves at the call sites, so the light
  // physically lives between plates.
  //
  // Binding plateEmissive here was tried and is WRONG: this material is also
  // what the visor, the chest core and the joint pips are made of, and those are
  // small dedicated lenses whose UVs have no reason to land on a lit groove of a
  // shared atlas — they would simply go dark. An unmapped emissive is correct
  // for a lens and incorrect for a plate, and the library cannot tell which it
  // has been given.
  //
  // What this file CAN do is stop paying for area with intensity. 6.5 was set
  // when a "glow" was a whole panel; the per-group overrides in RobotBuilder
  // (visor 12, core 4.4, spine 3.0) are the numbers that actually ship, and this
  // base only reaches a group nobody overrode. At 4.8 an unnamed glow is bright
  // enough to bloom and no longer bright enough to read as a lit surface the
  // size of a chest plate.
  ...
  const lib = {
    ...
    carbon, rubber, gasket, underskin, bezel, cable, glass, visor, emissive,
  };
